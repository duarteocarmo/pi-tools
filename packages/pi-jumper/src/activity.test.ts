...
test("summarizes path-based tools", () => {
  assert.equal(summarizeToolCall({ toolName: "read", args: { path: "/tmp/example.ts" } }), "read: /tmp/example.ts");
});
...
test("summarizes the latest user prompt", () => {
  assert.equal(summarizeUserPrompt({ prompt: "Fix picker\nand run tests" }), "Fix picker and run tests");
});
