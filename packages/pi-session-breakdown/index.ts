...
function setBorderedLoaderMessage(loader: BorderedLoader, message: string) {
  // BorderedLoader wraps a loader with setMessage(), but doesn't expose it publicly.
  const inner = (loader as unknown as { loader?: { setMessage?: unknown } }).loader;
  if (typeof inner?.setMessage === "function") inner.setMessage(message);
}

function recordFrom({ value }: { value: unknown }): JsonRecord | undefined {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return undefined;
  return value as JsonRecord;
}

function finiteNumberFrom({ value }: { value: unknown }): number {
...
function rangeFrom({ ranges, days }: { ranges: Map<number, RangeAgg>; days: number }): RangeAgg {
  const range = ranges.get(days);
  if (!range) throw new Error(`Missing ${days}-day range.`);
  return range;
}
...
function extractProviderModelAndUsage(obj: JsonRecord): {
  ...
  // Session format varies across versions and may nest values under message.
  const message = recordFrom({ value: obj.message });
  return {
...
function extractCostTotal(usage: unknown): number {
  const cost = recordFrom({ value: usage })?.cost;
  if (typeof cost === "number" || typeof cost === "string") return finiteNumberFrom({ value: cost });
  return finiteNumberFrom({ value: recordFrom({ value: cost })?.total });
}

function extractTokensTotal(usage: unknown): number {
  const values = recordFrom({ value: usage });
  if (!values) return 0;

  let total =
    finiteNumberFrom({ value: values.totalTokens }) ||
    finiteNumberFrom({ value: values.total_tokens }) ||
    finiteNumberFrom({ value: values.tokens }) ||
    finiteNumberFrom({ value: values.tokenCount }) ||
    finiteNumberFrom({ value: values.token_count });
  if (total > 0) return total;

  const tokens = recordFrom({ value: values.tokens });
  total =
    finiteNumberFrom({ value: tokens?.total }) ||
    finiteNumberFrom({ value: tokens?.totalTokens }) ||
    finiteNumberFrom({ value: tokens?.total_tokens });
  if (total > 0) return total;

  const input =
    finiteNumberFrom({ value: values.promptTokens }) ||
    finiteNumberFrom({ value: values.prompt_tokens }) ||
    finiteNumberFrom({ value: values.inputTokens }) ||
    finiteNumberFrom({ value: values.input_tokens });
  const output =
    finiteNumberFrom({ value: values.completionTokens }) ||
    finiteNumberFrom({ value: values.completion_tokens }) ||
    finiteNumberFrom({ value: values.outputTokens }) ||
    finiteNumberFrom({ value: values.output_tokens });
  const totalParts = input + output;
  return totalParts > 0 ? totalParts : 0;
}
...
async function parseSessionFile(filePath: string, signal?: AbortSignal): Promise<ParsedSession | null> {
  ...
  try {
    for await (const line of rl) {
      ...
      const mk =
        modelKeyFromParts(provider, model) ?? modelKeyFromParts(provider, modelId) ?? currentModel ?? "unknown";
      modelsUsed.add(mk);
...
async function computeBreakdown(
  ...
  const range90 = rangeFrom({ ranges, days: 90 });
  ...
  for (const filePath of candidates) {
    ...
    for (const d of RANGE_DAYS) {
      const range = rangeFrom({ ranges, days: d });
  ...
  const range30 = rangeFrom({ ranges, days: 30 });
...
class BreakdownComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    const range = rangeFrom({ ranges: this.data.ranges, days: selectedDays });
  ...
  pi.registerCommand("session-breakdown", {
    ...
    handler: async (_args, ctx: ExtensionContext) => {
      if (!ctx.hasUI) {
        ...
        const range = rangeFrom({ ranges: data.ranges, days: 30 });
        pi.sendMessage(
          {
